...
module.exports = function () {
  ...
  this.cleanUserCredentials = function(user, callback) {
    try {
      if (user._source !== undefined && user._source.password !== undefined) {
  ...
  this.encryptCredentials = function(requestObject, callback) {
    ...
    // If password is set to '*****', it means that we are updating a user with hidden password
    // => we do not update its password.
    if (requestObject.data.body.password === '*****') {
      delete requestObject.data.body.password;
      return callback(null, requestObject);
    }
